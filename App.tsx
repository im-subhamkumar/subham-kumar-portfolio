...
const App: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-700 selection:bg-amber-500 selection:text-white overflow-x-hidden ${isDarkMode ? 'bg-[#080807] text-slate-100' : 'bg-slate-50 text-slate-800'}`}>
      ...
      <AnimatePresence>
        {showFloatingButton && (
          <motion.a
            ...
            whileTap={{ scale: 0.95 }}
            className="fixed bottom-4 right-4 sm:bottom-10 sm:right-10 z-[60] flex items-center gap-3 sm:gap-4 px-4 py-3 sm:px-6 sm:py-4 glass-card rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.2)] border-slate-200 dark:border-white/10 group bg-white/80 dark:bg-black/80 backdrop-blur-md"
          >
            <span className="font-black uppercase tracking-widest text-[9px] sm:text-[10px] text-slate-700 dark:text-white hidden sm:inline">Resume</span>
      ...
      <footer className="py-12 text-center text-slate-400 dark:text-slate-500 text-xs font-bold uppercase tracking-widest border-t border-slate-200 dark:border-white/5 mt-20">
